...
- Tracks how many OpenAI tokens each user consumes because D-ID cannot track how much token is being used
- Sends per-message token usage to backend (PHP or Node)
...
This system also tracks how many OpenAI tokens each user consumes.

### How it works:

1. Every GPT call returns token usage:
```json
{
  "prompt_tokens": 12,
...
```
2. The FastAPI backend extracts this usage.

3. The backend sends the usage to your PHP backend using:
```bash
POST /save-token-usage
```
4. PHP stores the data in MySQL/PostgreSQL under that user's session.
...
- Usage analytics
- Prevent abuse
- Track how much each customer spends

Nothing changes on the frontend — all token reporting happens between FastAPI → PHP.
...
```
**This endpoint also collects token usage and sends it to PHP backend.**
...
- Token usage is generated by GPT, collected by FastAPI, and forwarded to the PHP backend for analytics/billing
