...
app = FastAPI(title="GPT + D-ID Presenter Realtime Chat")
...
def chat(req: ChatRequest):
    ...
    try:
        talk_id = create_did_video(reply_text)
        print("D-ID talk_id:", talk_id)
    except Exception as e:
        print("D-ID create error:", e)
        raise HTTPException(status_code=500, detail=f"D-ID create error: {e}")

    try:
        video_url = poll_did_video(talk_id)
        print("D-ID final URL:", video_url)
    except Exception as e:
        print("D-ID poll error:", e)
        raise HTTPException(status_code=500, detail=f"D-ID poll error: {e}")
